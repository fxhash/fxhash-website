...
const validation = Yup.object({
  ...
  pricingDutchAuction: YupPricingDutchAuction(
    0,
    "Must be after now"
  ).nullable(),
})
...
export function EditPricing({ token }: Props) {
  ...
  const disabled =
    ...
    (token.pricingDutchAuction &&
      isAfter(new Date(), new Date(token.pricingDutchAuction.opensAt!)))
      ...
      if (opensAtInput && isAfter(now, new Date(token.mintOpensAt!))) {
        ...
        if (isAfter(opensAtInput, now)) {
          ...
          // otherwise check if the values are different
            ...
            opensAtInput.getTime() !==
            new Date(token.pricingFixed.opensAt).getTime()
          ) {
            return true
          }
          // no abuse
              ...
              <>
                ...
                {warningTopGalleryAbuse(values.pricingFixed.opensAt) && (
                  <>
                    <Spacing size="small" />
                    <TextWarning>
                      Warning: only reschedule opening in case of an issue with
                      the project or if you're significantly modifying the allow
                      list. Rescheduling an opening time will push your token at
                      the top of the gallery, it is considered as an abuse and
