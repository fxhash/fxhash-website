 ...
 */
export function LiveMintingGuard({ children }: Props) {
  ...
  const error = useMemo(() => {
    ...
    if (liveMinting.error) {
      ...
      return {
        ...
        message: <LiveMintingWait event={liveMinting.event!} />,
      }
  ...
  return (
    <>
      {liveMinting.loading ? (
        <LoaderBlock size="small" height="80vh">
          loading mint pass
        </LoaderBlock>
      ) : error ? (
        <ErrorPage title={error.title}>{error.message}</ErrorPage>
      ) : (
        children
      )}
    </>
