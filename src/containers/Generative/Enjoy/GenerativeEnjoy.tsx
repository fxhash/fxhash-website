...
export function GenerativeEnjoy({
  ...
  const filteredTokens = useMemo(() => {
    return tokens.filter((token) => {
      if (!token?.issuer?.labels) return true
      const warning = getGenTokWarning(token.issuer.labels, settings)
      return !warning
    })
  }, [settings, tokens])
  const cursorShifted = (shift: number = 1) =>
    (cursorRef.current + shift + filteredTokens.length) % filteredTokens.length

  // has user interacted with the page since 2s ?
  const [isUserActive, setUserActive] = useHasInterractedIn(2000)

  const [timePerIteration, setTimePerIteration] = useState<number>(
    DEFAULT_TIME_PER_ITERATION_MS / 1000
  )
  ...
  const sortedTokens = useMemo<Objkt[]>(
    () =>
      sortRandom
        ? [
            ...filteredTokens.slice(0, cursor + 1),
            ...filteredTokens
              .slice(cursor + 1, filteredTokens.length)
              .sort(() => Math.random() - 0.5),
          ]
        : filteredTokens,
    [sortRandom, filteredTokens]
  )
      ...
      if (!paused) {
        ...
        if (barRef.current) {
          barRef.current.style.width = `${
            (relativeTimer.current / timeIterationMs) * 100
          }%`
        }

        if (
          relativeTimer.current > timeIterationMs - TRANSITION_DURATION_MS &&
          !frameContainerRef.current?.classList.contains(style.hidden) &&
          sortedTokens.length > 1
        ) {
          frameContainerRef.current?.classList.add(style.hidden)
    ...
    <main className={cs(style.root, { [style.no_cursor]: hideUI })}>
      ...
      >
        {loading ? (
          ...
          <div className={cs(style.empty)}>This collection is empty</div>
        )}
      </div>
      ...
      {sortedTokens.length > 0 && (
        <footer className={cs(style.footer, { [style.hide]: hideUI })}>
          ...
          <div className={cs(style.controls)}>
            <button onClick={() => shiftIteration(-1)} title="previous">
            ...
            <button onClick={() => shiftIteration(1)} title="next">
      ...
      <div className={cs(style.progress)}>
        <div ref={barRef} className={cs(style.bar)} />
      </div>
