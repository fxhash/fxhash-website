...
export function RevealProgress({ hash, onRevealed }: Props) {
  ...
  const { progress, error, success, loading, data, start } = useEventSource<
    SigningData,
    SigningProgress
  >(`${process.env.NEXT_PUBLIC_API_INDEXER}mint-feedback/${hash}`, (data) =>
    JSON.parse(data)
  )
  ...
  const reveal = () => {
    if (success) {
      ...
      if (!data || !data.cidGenerative || !data.cidPreview) {
        ...
        onRevealed(
          getIpfsSlash(data.cidGenerative),
          getIpfsSlash(data.cidPreview),
          data.features || []
        )
      }
    }
  ...
  return (
    <>
      ...
      <div className={cs(style.container)}>
        ...
        <p>
          fxhash is currently assigning the metadata of your token, please wait
          a little
        </p>
        ...
        <ProgressModule
          entries={[
            ...
            `token is in the queue ${
              progress?.state === SigningState.QUEUED
                ? `(position: ${progress?.extra?.position || "unknown"})`
                : ""
            }`,
            "server is generating your token metadata",
            `metadata generated ${
              progress?.state === SigningState.METADATA_GENERATED
                ? `(waiting to be signed, position: ${
                    progress?.extra?.position || "unknown"
                  })`
                : ""
            }`,
