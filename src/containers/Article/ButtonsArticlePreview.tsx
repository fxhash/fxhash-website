...
import {
  MintArticleOperation,
  TMintArticleOperationParams,
} from "../../services/contract-operations/MintArticle"
...
const _ButtonsArticlePreview = ({
  id,
  article,
}: ButtonsArticlePreviewProps) => {
  ...
  const handleClickMint = useCallback(async () => {
    const metadata = {
      thumbnailCid:
        article.thumbnailUri && ipfsCidFromUriOrCid(article.thumbnailUri),
      ...
      mintArticle({
        data: {
          ...
          distribution: {
            ...
            royalties_split: article.royaltiesSplits.map((split) => ({
  ...
  const canMintArticle = useMemo<boolean>(() => {
    if (
      ...
      countWords(article.metadata.description) > 500 ||
      article.royaltiesSplits.length < 1 ||
      article.royalties < 0 ||
      article.royalties > 250 ||
      article.editions < 1
    )
      return false
    return true
  ...
  return (
    <div className={style.container}>
      {!canMintArticle && (
        <Error>
          Before minting your article, you need to fix remaining errors in the
          editor
        </Error>
      )}
