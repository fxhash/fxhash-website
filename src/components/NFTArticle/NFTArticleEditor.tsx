...
const NftArticleEditor = forwardRef<RefNFTArticleEditor, NFTArticleEditorProps>(({
  ...
  const [initialValueSlate, setInitialValueSlate] = useState<Descendant[] | null>(null);

  const handleTransformMdToSlate = useCallback(async (mdValue) => {
    const getSlateEditorStateFromMarkdown = (await import('./processor')).getSlateEditorStateFromMarkdown
    const editorFromMd = mdValue ? await getSlateEditorStateFromMarkdown(mdValue) : null;
    setInitialValueSlate(editorFromMd ? editorFromMd.editorState : editorDefaultValue);
  }, []);
  const handleChangeSlate = useCallback(async (nodes: Descendant[]) => {
    const getMarkdownFromSlateEditorState = (await import('./processor')).getMarkdownFromSlateEditorState
    const markdown = await getMarkdownFromSlateEditorState(nodes);
    onChange(markdown || '');
  }, [onChange]);
  const handleToggleView = useCallback((view: View) => async () => {
    if (view === 'wysiwyg') {
  ...
  const handleUpdateMediaUrl = useCallback((target: IEditorMediaFile, uri: string) => {
    if (view === 'wysiwyg') {
  ...
  const handleUpdateMarkdownMedias = useCallback((ev) => {
    onMediasUpdate(getMarkdownMedias(ev.target.value))
  }, [onMediasUpdate])
  const debouncedUpdateMarkdownMedias = useMemo<Debounce<typeof handleUpdateMarkdownMedias>>(
    () => debounce(handleUpdateMarkdownMedias, 2000), [handleUpdateMarkdownMedias])

  const handleChangeMarkdown = useCallback((ev) => {
    onChange(ev.target.value);
    debouncedUpdateMarkdownMedias(ev);
  }, [debouncedUpdateMarkdownMedias, onChange])
  const handleBlurMarkdown = useCallback((ev) => {
  ...
  const debouncedChangeBody = useMemo(() => debounce(handleChangeSlate, 800), [handleChangeSlate])
  return (
    <>
      {initialValueSlate ?
        <div>
          ...
          <div className={cs(style.container_markdown, {
            ...
            {view === 'markdownWithPreview' &&
              <div className={style.container_preview_child}>
                <div className={cs(style.container_preview, stylePageArticle.body)}>
