...
import { Range, NodeEntry, Location } from "slate"
...
const FloatingInlineMenu = () => {
  ...
  const { inlineMenu } =
    BlockDefinitions[elementUnderCursor?.type as any as EArticleBlocks] || {}
  ...
  useClientEffect(() => {
    ...
    try {
      ...
      menuElement.style.top = `${
        rect.top + window.pageYOffset - menuElement.offsetHeight
      }px`
  ...
  useClientEffect(() => {
    if (!activeElement) setOverrideContent(null)
  }, [activeElement])
  ...
  return ReactDOM.createPortal(
    ...
    >
      {(overrideContent &&
        React.cloneElement(overrideContent, {
        ...
        <>
          {Object.keys(InlineMenuServices).map((key: string) => {
            ...
            if (Array.isArray(inlineMenu) && !inlineMenu.includes(key))
              return null
            const Service = InlineMenuServices[key as InlineServiceKey] as any
            return <Service key={key} setOverrideContent={setOverrideContent} />
          })}
        </>
      )}
    </div>,
