 ...
 */
export function MintingState({ token, verbose = false, existingState }: Props) {
  ...
  return (
    <>
      {state.activeState === EMintingState.DUTCH_AUCTION ? (
        <DutchAuctionState state={state} verbose={verbose} />
      ) : state.activeState === EMintingState.FIXED_PRICING ? (
        <FixedPricingState state={state.fixedPricingState!} verbose={verbose} />
      ) : state.activeState === EMintingState.MOD_LOCK ? (
        <ModLockState state={state.modLockState} verbose={verbose} />
      ) : null}

      {state.activeState !== null && <Spacing size="x-small" />}
    </>
  )
}
