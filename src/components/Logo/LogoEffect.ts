...
class Walker {
  ...
  constructor(x: number, y: number, ang: number, colX: number, colY: number) {
    ...
    this.y = y
    this.ang = ang
    this.colX = clamp(colX, 0, 1),
    this.colY = clamp(colY, 0, 1)
    this.setColor()
  }

  setColor() {
    this.col = [
      lerp(lerp(colors[0][0], colors[1][0], this.colX), colors[2][0], this.colY),
      lerp(lerp(colors[0][1], colors[1][1], this.colX), colors[2][1], this.colY),
      lerp(lerp(colors[0][2], colors[1][2], this.colX), colors[2][2], this.colY),
    ]
  }
}
...
export class LogoEffect {
  ...
  update = () => {
    ...
    for (let i = this.walkers.length-1; i >= 0; i--) {
      ...
      if (Math.random() < cfg.terminationChances || this.env[idx] > cfg.terminationThreshold
       || W.x < -1 || W.x >= this.cvs.width || W.y < -1 || W.y >= this.cvs.height) {
