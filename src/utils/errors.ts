...
const fileUploadErrors: Record<FileUploadError, string> = {
  ...
  NO_INDEX_HTML:
    "The zip file you uploaded does not have an index.html at its root",
  NO_SNIPPET:
    "The fxhash code snippet is missing from your index.html file, you can look at the guide to fix the issue",
...
const previewErrors: Record<PreviewError, string> = {
  ...
  AUTH_FAILED:
    "Server could not authenticate your project. Try again, but you may have to start over... sorry",
  INCORRECT_PARAMETERS: "Capture parameters are incorrect",
  PREVIEW_ERROR: "The server was not able to generate the preview.",
  IPFS_ERROR:
    "Error when trying to upload your preview on IPFS. Please try again.",
}
...
const mintErrors: Record<MintError, string> = {
  UNKNOWN:
    "Unkown error. Sorry for the lack of informations, but something unexpected happened 😟",
  ...
  FAIL_GET_TOKEN:
    "Could not get the Generative Token data, please try again later",
...
const processRawFeaturesErrors: Record<
  ProcessRawTokenFeatureErrorType,
  string
> = {
  UNKNOWN:
    "Unknown error, please verify if your implementation follows the specs in our guide, and if the error persist you can reach out in the Discord server",
  INVALID_FEATURES_SIGNATURE:
    "The type of the features object your token returned is invalid, please verify if your implementation follows the specs in our guide",
  INVALID_PROPERTY_TYPE: "The type of one of the properties is invalid:",
}

export function getProcessRawFeaturesError(
  type: ProcessRawTokenFeatureErrorType
) {
  return (
    processRawFeaturesErrors[type] ||
    processRawFeaturesErrors[ProcessRawTokenFeatureErrorType.UNKNOWN]
  )
}
